...
public static class RecitationPracticeSettings
{
    ...
    public static int GetAyahCount(int surah)
    {
    ...
    public static List<int> BuildAyahNumbers(int surah)
    {
        var ayahCount = GetAyahCount(surah);
        return Enumerable.Range(1, ayahCount).ToList();
    }

    public static int ClampAyah(int selectedAyah, int surah)
    {
        var maxAyah = GetAyahCount(surah);
        return Math.Clamp(selectedAyah, 1, maxAyah);
    }

    public static bool ShouldShowAdvancedPanel(bool isAdvancedMode, bool panelState)
        => isAdvancedMode && panelState;
}
